...
def cross_validate(X: ArrayLike,
    ...
    for it in tqdm(range(conf.EVAL.N_REPEATS), total=conf.EVAL.N_REPEATS):
        ...
        for i, (train_idx, test_idx) in tqdm(enumerate(outer_cv.split(X_sh, y_sh)), total=outer_cv.get_n_splits(X_sh, y_sh), leave=False):
            ...
            times["feat_selection"].append(feat_selection_end - feat_selection_start)
            ...
            times["scaling"].append(scaling_end - scaling_start)

            training_start = time.time()
            # fit model using the reduced feature set
            model.fit(Xtrain.loc[:, feat_subset], ytrain)
            training_end = time.time()
            times["training"].append(training_end - training_start)
            ...
            times["calibration"].append(calibration_end - calibration_start)
            
            #models.append(calib_model)
            
            prediction_val_start = time.time()
            test_preds = calib_model.predict_proba(Xtest.loc[:, feat_subset])
            prediction_val_end = time.time()
            times["prediction_val"].append(prediction_val_end - prediction_val_start)
            ...
            times["prediction_train"].append(prediction_train_end - prediction_train_start)
            ...
            find_errors_end = time.time()
            times["find_errors"].append(find_errors_end - find_errors_start)
            
            overall_end = time.time()
            times["overall"].append(overall_end - overall_start)
...
def cross_validate_loocv(X: ArrayLike,
                   y: ArrayLike,
                   outer_cv: CVScheme,
                   inner_cv: CVScheme,
                   conf: CfgNode) -> List[Tuple[Estimator, Dict, Dict]]:
    ...
    for it in tqdm(range(conf.EVAL.N_REPEATS), total=conf.EVAL.N_REPEATS):
        ...
        for i, (train_idx, test_idx) in tqdm(enumerate(outer_cv.split(X_sh, y_sh)), 
                                             total=outer_cv.get_n_splits(X_sh, y_sh), 
                                             leave=False):
            overall_start = time.time()
            # Xtemp.shape[0] will be len(X_sh)-1
            Xtemp, ytemp = X_sh.iloc[train_idx, :], y_sh.iloc[train_idx]
            
            # Xtest.shape[0] will be 1
            Xtest, ytest = X_sh.iloc[test_idx, :], y_sh.iloc[test_idx]
