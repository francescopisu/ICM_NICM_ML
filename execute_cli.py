...
from src.evaluate import cross_validate, get_evaluation_metrics, test_performance, cross_validate_loocv
...
@click.option('--cv_type', default="10-fold")
def cross_validation(exp_name, cv_type):
    """
    ...
    if cv_type == 'loo':
        outer_cv = KFold(n_splits=X_train.shape[0],
                         shuffle=defaults.EVAL.SHUFFLE,
                         random_state=defaults.MISC.SEED)
        cv_logic = cross_validate_loocv
    else:
        outer_cv = StratifiedKFold(n_splits=defaults.EVAL.N_OUTER_SPLITS,
                                    shuffle=defaults.EVAL.SHUFFLE,
                                    random_state=defaults.MISC.SEED)              
        cv_logic = cross_validate                   
    
    s = time.time()
    
    outer_results, outer_preds = cv_logic(X=X_train, y=y_train,
                                                outer_cv=outer_cv,
                                                inner_cv=inner_cv,
                                                conf=defaults)
